...
# Add list of all game files
GAME_SCRIPTS := $(shell find ./scripts -name "*.c" -print)

# Set the MCU accordingly to your device (e.g. at90usb1286 for a Teensy 2.0++, or atmega16u2 for an Arduino UNO R3)
MCU          = atmega32u4 # Arduino Miro
ARCH         = AVR8
F_CPU        = 16000000
F_USB        = $(F_CPU)
OPTIMIZATION = s
TARGET       = Joystick
SRC          = $(TARGET).c Descriptors.c clock.c $(GAME_SCRIPTS) $(LUFA_SRC_USB)
LUFA_PATH    = ./lufa/LUFA
